...
v1.2, whatever the README says today. If you ever want to know whether there's a newer one, it's at
the top of the README on GitHub."* Nothing is retrieved from anywhere and no upgrade is offered:
there is no update mechanism and this line is not the start of one. It exists because after install
nobody opens their own README again, so this is the only place the number is ever reachable.
